...
void TestSyntheticRendering(IDirect3DDevice9* device, const std::filesystem::path& output)
{
    ...
    const auto ordinaryOverlap = Draw(device, &overlappingSea, context, view, projection,
        output / "synthetic-layered-water.bmp");
    Check(ChangedPixels(singleSheet, ordinaryOverlap) > 20000,
        "authored overlapping water layers retain their separate blended contributions");
    ...
    Check(ChangedPixels(singleSheet, transparentFirst, 0) == 0,
        "a zero-alpha water layer leaves the visible layer behind it intact");
...
void TestRiverPlacement(const noesisModel_t& model)
{
    // Independently decoded ka01 river and MZB bed fixture in the raw Y-down frame.
    float waterY = 0, bedY = 0;
    bool foundWater = false, foundBed = false;
    for (const auto& mesh : model.submeshes)
    {
        const bool water = mesh.water && mesh.water->generator == "ka01";
        const bool bed = !mesh.water && mesh.objectName.find("kaws_a4_m") != std::string::npos;
        if (!water && !bed) continue;
        ...
        for (size_t i = 0; i + 2 < mesh.cpuIndices.size(); i += 3)
        {
            ...
            if (water) { waterY = y; foundWater = true; }
            if (bed) { bedY = y; foundBed = true; }
            break;
        }
    }
    Check(foundWater && foundBed && Near(waterY, -48.0862427f, 0.001f) && Near(bedY, -47.75f, 0.001f) && waterY < bedY,
        "river placement matches the decoded world height and sits above its authored terrain bed");
}
                      ...
                      const std::filesystem::path& output, const InstalledFixture& fixture)
{
    ...
    if (std::string(fixture.name) == "east-ronfaure") TestRiverPlacement(*model);
    ModelRenderer::Context context;
    context.rendersZoneObjects = true;
