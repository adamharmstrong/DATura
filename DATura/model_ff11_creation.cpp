	...
	std::vector<Creation_ShapeSkin> &outSkins)
{
	...
	for (int searchOfs = 0;;)
	{
		...
		const int declaredCount = Creation_ReadI32(pData, dataSize, chunkOfs + 100);
		if (clusterCount <= 0 || clusterCount > 1024 || declaredCount <= 0)
			continue;

		// The second word is not the shape's vertex count (for example, the
		// 933-vertex Hume body declares 510 here). Size the influence table from
		// the actual indices so vertices above that unrelated field are skinned.
		Creation_ShapeSkin skin;
		...
		for (int clusterIndex = 0; clusterIndex < clusterCount && valid; ++clusterIndex)
		{
			...
			cursor += 8;
			if (boneIndex < 0 || influenceCount < 0 || influenceCount > 1000000 ||
			...
			for (int influenceIndex = 0; influenceIndex < influenceCount; ++influenceIndex)
			{
				...
				if (vertexIndex >= 0 && vertexIndex < 1000000 && weight > 0.0f)
				{
					if ((size_t)vertexIndex >= skin.size())
						skin.resize((size_t)vertexIndex + 1);
	...
	const int boneCount, const float pos[3], const float nrm[3])
{
	...
	for (int weightIndex = 0; weightIndex < skin.weightCount; ++weightIndex)
	{
		...
		for (int axis = 0; axis < 3; ++axis)
		{
			// FFXI's CPU skinning path stores a separate, already-weighted
			// position contribution for each influence. Translation is weighted
			// later through the homogeneous component, so scale the local XYZ
			// here as well to produce a conventional weighted skin transform.
			skin.pos[weightIndex][axis] = localPos[axis] * skin.boneWt[weightIndex];
