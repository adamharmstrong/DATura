...
public:
								...
								const unsigned char *pChunkData, const int dataSize)
	{
		...
		CollectZoneVisibility(pDecrypted, dataSize, objectCount);
...
protected:
	static void CollectZoneVisibility(const unsigned char *pData, const int dataSize,
		const int objectCount)
	{
		const int meshOfs = ReadInt32LE(pData, dataSize, 0x08);
		const int treeRootOfs = ReadInt32LE(pData, dataSize, 0x10);
		if (!RangeIsValid(meshOfs, 0x20, dataSize) ||
			!RangeIsValid(treeRootOfs, 0x80, dataSize))
			return;

		const int recordListOfs = ReadInt32LE(pData, dataSize, meshOfs + 0x14);
		int recordCount = ReadInt32LE(pData, dataSize, meshOfs + 0x18);
		const int recordStride = 0xC0;
		if (recordCount <= 0 || recordCount > objectCount ||
			!RangeIsValid(recordListOfs, recordCount * recordStride, dataSize))
			return;

		gFF11LastZoneVisibilityRecords.resize((size_t)recordCount);
		std::vector<unsigned int> tableOffsets;
		for (int recordIndex = 0; recordIndex < recordCount; ++recordIndex)
		{
			const int recordOfs = recordListOfs + recordIndex * recordStride;
			const unsigned int tableOffset =
				(unsigned int)ReadInt32LE(pData, dataSize, recordOfs + 0xA8);
			gFF11LastZoneVisibilityRecords[(size_t)recordIndex].cullingTableOffset = tableOffset;
			if (tableOffset && std::find(tableOffsets.begin(), tableOffsets.end(), tableOffset) == tableOffsets.end())
				tableOffsets.push_back(tableOffset);
		}

		for (unsigned int tableOffset : tableOffsets)
		{
			if (!RangeIsValid((int)tableOffset, 4, dataSize))
				continue;
			const int visibleCount = ReadInt32LE(pData, dataSize, (int)tableOffset);
			if (visibleCount < 0 || visibleCount > recordCount ||
				!RangeIsValid((int)tableOffset + 4, visibleCount * 4, dataSize))
				continue;
			ff11ZoneVisibilityTable_t table = {};
			table.offset = tableOffset;
			table.visibleRecordIndices.reserve((size_t)visibleCount);
			for (int visibleIndex = 0; visibleIndex < visibleCount; ++visibleIndex)
			{
				const unsigned int recordIndex = (unsigned int)ReadInt32LE(
					pData, dataSize, (int)tableOffset + 4 + visibleIndex * 4);
				if (recordIndex < (unsigned int)recordCount)
					table.visibleRecordIndices.push_back(recordIndex);
			}
			gFF11LastZoneVisibilityTables.push_back(std::move(table));
		}

		std::vector<int> pendingNodes(1, treeRootOfs);
		std::vector<int> visitedNodes;
		while (!pendingNodes.empty() && visitedNodes.size() < 65536)
		{
			const int nodeOfs = pendingNodes.back();
			pendingNodes.pop_back();
			if (!RangeIsValid(nodeOfs, 0x80, dataSize) ||
				std::find(visitedNodes.begin(), visitedNodes.end(), nodeOfs) != visitedNodes.end())
				continue;
			visitedNodes.push_back(nodeOfs);

			const int visibilityListOfs = ReadInt32LE(pData, dataSize, nodeOfs + 0x60);
			const int visibilityCount = ReadInt32LE(pData, dataSize, nodeOfs + 0x64);
			if (visibilityCount > 0 && visibilityCount <= recordCount &&
				RangeIsValid(visibilityListOfs, visibilityCount * 4, dataSize))
			{
				ff11ZoneVisibilityLeaf_t leaf = {};
				for (int vertexIndex = 0; vertexIndex < 8; ++vertexIndex)
				{
					for (int axis = 0; axis < 3; ++axis)
					{
						const float value = ReadFloatLE(pData, dataSize,
							nodeOfs + (vertexIndex * 3 + axis) * 4);
						if (vertexIndex == 0)
							leaf.boundsMin[axis] = leaf.boundsMax[axis] = value;
						else
						{
							if (value < leaf.boundsMin[axis]) leaf.boundsMin[axis] = value;
							if (value > leaf.boundsMax[axis]) leaf.boundsMax[axis] = value;
						}
					}
				}
				for (int listIndex = 0; listIndex < visibilityCount; ++listIndex)
				{
					const unsigned int recordIndex = (unsigned int)ReadInt32LE(
						pData, dataSize, visibilityListOfs + listIndex * 4);
					if (recordIndex < (unsigned int)recordCount)
						leaf.recordIndices.push_back(recordIndex);
				}
				if (!leaf.recordIndices.empty())
					gFF11LastZoneVisibilityLeaves.push_back(std::move(leaf));
			}

			for (int childIndex = 0; childIndex < 4; ++childIndex)
			{
				const int childOfs = ReadInt32LE(pData, dataSize, nodeOfs + 0x68 + childIndex * 4);
				if (childOfs)
					pendingNodes.push_back(childOfs);
			}
		}
	}
...
public:
	...
	struct SMapGeoData
	{
		explicit SMapGeoData(const SMapGeoHeader *pMapGeoHdr, noeRAPI_t *pAllocatingInstance,
			const int dataSize, const char *directoryPath, const char *resourceName)
			: mpMapGeoHdr(pMapGeoHdr)
			, mpAllocatingInstance(pAllocatingInstance)
			, mDataSize(dataSize)
		{
			strcpy_s(mDirectoryPath, directoryPath ? directoryPath : "");
			memset(mResourceName, 0, sizeof(mResourceName));
			if (resourceName)
				memcpy(mResourceName, resourceName, 4);
			mResourceName[4] = 0;
			for (int index = 3; index >= 0 && mResourceName[index] == ' '; --index)
				mResourceName[index] = 0;
		}

		const SMapGeoHeader *mpMapGeoHdr;
		noeRAPI_t *mpAllocatingInstance;
		int mDataSize;
		char mDirectoryPath[128];
		char mResourceName[8];
	};
								...
								const unsigned char *pChunkData, const int dataSize)
	{
		...
		mMapGeoList.push_back(SMapGeoData(pMapGeoHdr, pRapi, dataSize,
			dat.GetChunkDirectoryPath(chunk), chunk.mName));
							...
							const ff11GeneratorRecord_t *pEnvironmentGenerator = NULL)
	{
		...
		else if (emitRender)
		{
			...
			if (geoData.mDirectoryPath[0] && pEnvironmentGenerator)
				...
				sprintf_s(nameString, "env: %s/@%s/%s", geoData.mDirectoryPath,
					geoData.mResourceName, objectName);
			else
		...
		while (MapGeoRangeFits(drawOfs, 1, sizeof(SDrawHeader), geoData.mDataSize))
		{
			...
			for (int superIndex = 0; superIndex < superEntryCount && !malformedMapGeo; ++superIndex)
			{
				...
				for (int subIndex = 0; subIndex < pSubHeader->mSegCount; ++subIndex)
				{
					...
					char rawMatName[CFFXITextureHandler::skTexNameLength + 1];
					memcpy(rawMatName, pMatName, CFFXITextureHandler::skTexNameLength);
					rawMatName[CFFXITextureHandler::skTexNameLength] = 0;
					Model_FF11_BuildScopedResourceName(matName, sizeof(matName),
						geoData.mDirectoryPath, rawMatName);
...
protected:
	...
	{
		(void)transform;
		// Reject malformed strip bridges in the mesh's authored coordinate space.
		// Placement transforms must not participate in this test: environment
		// controllers legitimately scale their cloud shells by roughly 5-20x.
		// Testing the transformed points discarded nearly every normal triangle in
		// those shells and left only the short-edge fragments as a jagged horizon
		// ribbon.
		const float *p0 = (const float *)(pVertData + idx0 * vertStride + posOfs);
		const float *p1 = (const float *)(pVertData + idx1 * vertStride + posOfs);
		const float *p2 = (const float *)(pVertData + idx2 * vertStride + posOfs);

		auto distSq = [](const float *a, const float *b)
		{
